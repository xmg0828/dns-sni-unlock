...
CONFIG_DIR="/etc/dnsmasq.d"; DNSMASQ_CONFIG="/etc/dnsmasq.conf"; SNIPROXY_CONFIG="/etc/sniproxy.conf"
SERVICE_CONFIG="/root/dns_unlock_services.conf"; WHITELIST_FILE="/root/firewall_whitelist_ips.txt"
DEFAULT_IP="127.0.0.1"; PROTECTED_PORTS=("53" "80" "443")

check_root() { [ "$(id -u)" != "0" ] && echo -e "${RED}错误: 请使用root权限运行此脚本!${PLAIN}" && exit 1; }

check_system() {
  if [ -f /etc/redhat-release ]; then RELEASE="centos"
  elif grep -Eqi "debian|ubuntu" /etc/issue || grep -Eqi "debian|ubuntu" /proc/version; then RELEASE="debian"
  else echo -e "${RED}系统不支持，请使用CentOS/Debian/Ubuntu系统！${PLAIN}" && exit 1; fi
}

install_packages() {
  echo -e "${BLUE}开始安装必要软件...${PLAIN}"
  if [ "${RELEASE}" == "centos" ]; then
    yum update -y && yum install -y epel-release bind-utils dnsmasq curl wget git make gcc openssl-devel libevent-devel iptables iptables-services sniproxy
  else
    apt-get update -y && apt-get install -y dnsutils dnsmasq curl wget git build-essential libudns-dev libev-dev libpcre3-dev iptables iptables-persistent sniproxy
  fi
...
resolver { nameserver 8.8.8.8; nameserver 8.8.4.4; mode ipv4_only }
listener 0.0.0.0:80 { proto http; access_log { filename /var/log/sniproxy/http_access.log; priority notice } }
listener 0.0.0.0:443 { proto tls; access_log { filename /var/log/sniproxy/https_access.log; priority notice } }
table {
    .*tiktok\.com$ *
    .*tiktokcdn\.com$ *
    .*netflix\.com$ *
...
EOF
  systemctl daemon-reload && echo -e "${GREEN}服务文件创建完成!${PLAIN}"
}

fix_sniproxy() {
  echo -e "${BLUE}尝试修复SNIProxy...${PLAIN}"
  systemctl stop sniproxy && config_sniproxy && create_service_files
  systemctl restart sniproxy && systemctl enable sniproxy
  if systemctl is-active sniproxy &> /dev/null; then
    echo -e "${GREEN}SNIProxy已修复并成功启动!${PLAIN}"
  else
    echo -e "${RED}SNIProxy启动失败，请检查日志: journalctl -u sniproxy${PLAIN}"
  fi
}
...
TikTok:${DEFAULT_IP}:tiktok.com tiktok.org bytedance.com tiktokv.com tiktokcdn.com musical.ly lemon8-app.com capcut.com muscdn.com tiktokcdn-us.com
OpenAI:${DEFAULT_IP}:openai.com chat.openai.com platform.openai.com api.openai.com auth0.openai.com chatgpt.com oaistatic.com oaiusercontent.com cdn.oaistatic.com
...
apply_service_config() {
  ...
  while IFS=':' read -r service ip domains || [ -n "$service" ]; do
    ...
    echo "# === $service 解锁规则 ===" > "$conf_file"
    for domain in $domains; do echo "address=/$domain/$ip" >> "$conf_file"; done
...
apply_firewall_rules() {
  ...
  if [ "${RELEASE}" == "centos" ]; then service iptables save
  else netfilter-persistent save > /dev/null 2>&1 && netfilter-persistent reload > /dev/null 2>&1; fi
...
add_ip_to_whitelist() {
  ...
  if [ -z "$new_ip" ] || ! [[ $new_ip =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo -e "${RED}IP格式不正确!${PLAIN}" && return
  fi
  if grep -Fxq "$new_ip" "$WHITELIST_FILE"; then
    echo -e "${YELLOW}IP $new_ip 已存在于白名单中.${PLAIN}" && return
  fi
...
remove_ip_from_whitelist() {
  echo -e "${BLUE}当前白名单IP:${PLAIN}" && cat -n "$WHITELIST_FILE"
  read -p "请输入要移除的IP编号: " ip_num
  if ! [[ "$ip_num" =~ ^[0-9]+$ ]]; then echo -e "${RED}无效选择!${PLAIN}" && return; fi
  selected_ip=$(sed -n "${ip_num}p" "$WHITELIST_FILE")
  if [ -z "$selected_ip" ]; then echo -e "${RED}IP不存在!${PLAIN}" && return; fi
  read -p "确定要移除IP $selected_ip? (y/n): " confirm
  if [ "$confirm" != "y" ]; then echo -e "${GREEN}操作已取消${PLAIN}" && return; fi
...
manage_firewall() {
  ...
  case $option in
    1) echo -e "${YELLOW}当前白名单IP:${PLAIN}" && [ -f "$WHITELIST_FILE" ] && cat -n "$WHITELIST_FILE" || echo -e "${RED}白名单文件不存在!${PLAIN}" ;;
    2) add_ip_to_whitelist ;;
    3) remove_ip_from_whitelist ;;
    4) apply_firewall_rules ;;
    5) echo -e "${YELLOW}警告: 关闭防火墙将允许所有IP访问您的服务。${PLAIN}"
       read -p "确定要继续吗? (y/n): " confirm
       if [ "$confirm" = "y" ]; then
         iptables -F && iptables -X
         if [ "${RELEASE}" == "centos" ]; then service iptables save
         else netfilter-persistent save > /dev/null 2>&1; fi
         echo -e "${GREEN}防火墙已关闭!${PLAIN}"
       fi ;;
    0) return ;;
    *) echo -e "${RED}无效选项!${PLAIN}" ;;
  esac
  read -p "按任意键继续..." key && manage_firewall
}
...
restart_services() {
  ...
  systemctl restart dnsmasq && systemctl restart sniproxy
  echo -e "${GREEN}服务已重启!${PLAIN}"
}

check_status() {
  echo -e "${BLUE}服务状态:${PLAIN}"
  echo -e "${YELLOW}dnsmasq状态:${PLAIN}" && systemctl status dnsmasq --no-pager
  echo -e "${YELLOW}sniproxy状态:${PLAIN}" && systemctl status sniproxy --no-pager
...
change_service_ip() {
  ...
  local services=() && local i=1
  while IFS=':' read -r service ip domains; do
    [[ "$service" =~ ^#.*$ || -z "$service" ]] && continue
    services+=("$service")
    echo -e "$i. ${GREEN}$service${PLAIN} - 当前IP: ${YELLOW}$ip${PLAIN}" && ((i++))
  done < "$SERVICE_CONFIG"
  ...
  apply_service_config && restart_services
}

add_custom_service() {
  read -p "请输入服务名称: " service_name
  if [ -z "$service_name" ]; then echo -e "${RED}服务名称不能为空!${PLAIN}" && return; fi
  if grep -q "^$service_name:" "$SERVICE_CONFIG"; then echo -e "${RED}服务 $service_name 已存在!${PLAIN}" && return; fi
  read -p "请输入解锁IP (留空则使用本机IP 127.0.0.1): " service_ip
  [ -z "$service_ip" ] && service_ip="127.0.0.1"
  if ! [[ $service_ip =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then echo -e "${RED}IP格式不正确!${PLAIN}" && return; fi
  read -p "请输入域名列表 (多个域名用空格分隔): " service_domains
  if [ -z "$service_domains" ]; then echo -e "${RED}域名列表不能为空!${PLAIN}" && return; fi
...
add_domains_to_service() {
  ...
  while IFS=':' read -r service ip domains; do
    [[ "$service" =~ ^#.*$ || -z "$service" ]] && continue
    services+=("$service") && echo -e "$i. ${GREEN}$service${PLAIN}" && ((i++))
  done < "$SERVICE_CONFIG"
  ...
  if [ -z "$new_domains" ]; then echo -e "${RED}域名不能为空!${PLAIN}" && return; fi
...
reset_config() {
  ...
  systemctl stop dnsmasq && systemctl stop sniproxy
  rm -f $CONFIG_DIR/*.conf && rm -f "$SERVICE_CONFIG"
  config_dnsmasq && config_sniproxy && init_service_config && apply_service_config
  echo -e "${GREEN}配置已重置!${PLAIN}" && restart_services
}
...
uninstall_service() {
  ...
  systemctl stop dnsmasq && systemctl stop sniproxy
  systemctl disable dnsmasq && systemctl disable sniproxy
  rm -f /etc/systemd/system/sniproxy.service $CONFIG_DIR/*.conf "$SERVICE_CONFIG" "$SNIPROXY_CONFIG" "$WHITELIST_FILE"
  iptables -F && iptables -X
  if [ "${RELEASE}" == "centos" ]; then service iptables save
  else netfilter-persistent save > /dev/null 2>&1; fi
  systemctl daemon-reload && echo -e "${GREEN}解锁服务已卸载!${PLAIN}"
}

install_service() {
  check_root && check_system && install_packages
  config_dnsmasq && config_sniproxy && create_service_files
  init_service_config && apply_service_config && init_firewall_whitelist && apply_firewall_rules
  start_services
  if ! systemctl is-active sniproxy &> /dev/null; then
    echo -e "${YELLOW}SNIProxy启动失败，尝试修复...${PLAIN}" && fix_sniproxy
  fi
  echo -e "${GREEN}解锁服务安装完成!${PLAIN}" && check_status
}

repair_service() {
  echo -e "${BLUE}开始修复服务...${PLAIN}"
  systemctl stop dnsmasq && config_dnsmasq && systemctl restart dnsmasq && systemctl enable dnsmasq
  fix_sniproxy && apply_service_config && apply_firewall_rules
  echo -e "${GREEN}服务修复完成!${PLAIN}" && check_status
}
...
show_menu() {
  ...
  read -p "按任意键继续..." key && show_menu
}

main() { check_root && show_menu; }
main
